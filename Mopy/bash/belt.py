...
from __future__ import \
    with_statement  # Python 2.5 'with' statement compatability
...
class PageVersions(PageInstaller):
    def __init__(self, parent, bFoOk, foHave, foNeed, bNVSEOk, nvseHave,
        nvseNeed, bOBGEOk, obgeHave, obgeNeed, bWBOk, wbHave, wbNeed):
        PageInstaller.__init__(self, parent)
        ...
        linkNVSE = wx.HyperlinkCtrl(self, -1, 'Fallout Script Extender',
        ...
        linkWB = wx.HyperlinkCtrl(self, -1, 'Wrye Flash NV',
...
class WryeParser(ScriptParser.Parser):
    ...
    def fnCompareNVSEVersion(self, nvseWant):
        ...
        ret = self._TestVersion(self._TestVersion_Want(nvseWant),
            bosh.dirs['app'].join(nvse))
        return ret[0]
    ...
    def kwdRequireVersions(self, fo, nvse='None', obge='None', wbWant='0'):
        ...
        if not bFoOk or not bNVSEOk or not bOBGEOk or not bWBOk:
            self.page = PageVersions(self.parent, bFoOk, foHave, ob, bNVSEOk,
                nvseHave, nvse, bOBGEOk, obgeHave, obge, bWBOk, wbHave, wbWant)

    def _TestVersion_OBGE(self, want):
        retOBGEOld = self._TestVersion(want,
            bosh.dirs['mods'].join('nvse', 'plugins', 'obge.dll'))
        retOBGENew = self._TestVersion(want,
            bosh.dirs['mods'].join('nvse', 'plugins', 'obgev2.dll'))
