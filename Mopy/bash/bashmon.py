...
class Data:
    ...
    def update(self):
        ...
        if not self.savesDir or ini.mtime != ini.path.mtime:
            ...
            savesDir = bosh.dirs['saveBase'].join(
                ini.getSetting('General', 'SLocalSavePath', 'Saves\\'))
    ...
    def setSavesDir(self, savesDir):
        ...
        # --Pluggy text
        signalDir = bosh.dirs['saveBase'].join('Pluggy', 'User Files',
            'BashMon')
...
@monitorCommand
def ripAppearance(srcName, destName, srcForm='player', destForm='player',
...
@monitorCommand
def swapPlayer(saveName, oldForm, newForm,
...
def moveSave(oldSave, newSave):
    ...
    if oldPath.exists():
        if not oldPath.isfile(): raise bolt.BoltError(
            oldPath + _(' is not a save file.'))
