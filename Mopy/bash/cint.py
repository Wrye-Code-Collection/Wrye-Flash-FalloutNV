...
if CBashdll:
    try:
        ...
        if (_CGetVersionMajor(), _CGetVersionMinor(),
        _CGetVersionRevision()) < _CBashRequiredVersion:
            raise ImportError(
                "cint.py requires CBash v%d.%d.%d or higher! (found v%d.%d.%d)" % (
                _CBashRequiredVersion + (
                _CGetVersionMajor(), _CGetVersionMinor(),
                _CGetVersionRevision())))
...
FORMIDAttrs = {
    ...
    'REFR': ['base', 'destination', 'lockKey', 'owner', 'parent',
    ...
    'KEYM': ['script'],
    'LIGH': ['script', 'sound'],
    'MGEF': ['associated', 'effectShader', 'enchantEffect', 'castingSound',
        'boltSound', 'hitSound', 'areaSound'],
    'MISC': ['script'],
    'NPC_': ['deathItem', 'script', 'race', 'iclass', 'hair', 'eye',
        'combatStyle'],
    'RACE': ['maleVoice', 'femaleVoice', 'defaultHairMale',
        'defaultHairFemale'],
    'SGST': ['script'],
    'SLGM': ['script'],
...
def MakeShortFid(CollectionID, fid):
    ...
    if master is None:
        raise AttributeError(
            "Unable to convert long fid (None, %06X) into a short formID" % object)
...
def MakeShortMGEFCode(CollectionID, ModID, MGEFCode):
    if isinstance(MGEFCode, basestring): MGEFCode = cast(MGEFCode,
        POINTER(c_ulong)).contents.value
    if MGEFCode is None or MGEFCode == 0: return 0
    if not isinstance(MGEFCode, tuple): return MGEFCode
    master, object = MGEFCode
    if master is None:
        raise AttributeError(
            "Unable to convert long MGEFCode (None, %06X) into a short MGEFCode" % object)
    masterIndex = _CGetModLoadOrderByName(CollectionID, str(master))
    if (masterIndex == -1): return None
...
def ExtractCopyList(Elements):
    return [tuple(getattr(listElement, attr) for attr in listElement.copyattrs)
...
def ExtractExportList(Element):
    try:
        return [tuple(ExtractExportList(listElement) if hasattr(listElement,
            'exportattrs') else getattr(listElement, attr) for attr in
        listElement.exportattrs) for listElement in Element]
    except TypeError:
        return [tuple(ExtractExportList(getattr(Element, attr)) if hasattr(
            getattr(Element, attr), 'exportattrs') else getattr(Element, attr)
...
class CBashGrouped(object):
    ...
    def __get__(self, instance, owner):
        oElement = self._Type(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID)
        if (self._AsList): return tuple(
            [getattr(oElement, attr) for attr in oElement.copyattrs])
        return oElement
    ...
    def __set__(self, instance, nElement):
        oElement = self._Type(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID)
        if nElement is None:
            nValueList = tuple([None for attr in oElement.copyattrs])
        elif isinstance(nElement, tuple):
            nValueTuple = nElement
        else:
            nValueTuple = tuple(
                [getattr(nElement, attr) for attr in nElement.copyattrs])
        for nValue, attr in zip(nValueTuple, oElement.copyattrs):
            setattr(oElement, attr, nValue)
...
class CBashBasicType(object):
    ...
    def __set__(self, instance, nValue):
        if nValue:
            setattr(instance, self._AttrName, self._Value)
        else:
            setattr(instance, self._DefaultFieldName, True)
...
class CBashMaskedType(object):
    ...
    def __set__(self, instance, nValue):
        if nValue:
            ...
            setattr(instance, self._AttrName, field)
        elif getattr(instance, self._AttrName, None):
            setattr(instance, self._DefaultFieldName, True)
...
class CBashGeneric_GROUP(object):
    ...
    def __get__(self, instance, owner):
        ...
        retValue = _CGetField(instance._CollectionID, instance._ModID,
            instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0, 0)
    ...
    def __set__(self, instance, nValue):
        FieldID = self._FieldID + instance._FieldID
        if nValue is None:
            _CDeleteField(instance._CollectionID, instance._ModID,
                instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0)
        else:
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0,
                byref(self._Type(nValue)), 0)
...
class CBashUINT8ARRAY_GROUP(object):
    ...
    def __get__(self, instance, owner):
        FieldID = self._FieldID + instance._FieldID
        numRecords = _CGetFieldAttribute(instance._CollectionID,
            instance._ModID, instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0, 1)
        if (numRecords > 0):
            cRecords = POINTER(c_ubyte * numRecords)()
            _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0, byref(cRecords))
            return [cRecords.contents[x] for x in range(0, numRecords)]
        return []

    def __set__(self, instance, nValue):
        FieldID = self._FieldID + instance._FieldID
        if nValue is None or not len(nValue):
            _CDeleteField(instance._CollectionID, instance._ModID,
                instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0)
        else:
            length = len(nValue)
...
class CBashFLOAT32_GROUP(object):
    ...
    def __set__(self, instance, nValue):
        ...
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0,
                byref(c_float(round(nValue, 6))), 0)
...
class CBashSTRING_GROUP(object):
    ...
    def __set__(self, instance, nValue):
        ...
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, FieldID, 0, 0, 0, 0, 0, 0, str(nValue), 0)
...
class CBashLIST(object):
    ...
    def __get__(self, instance, owner):
        numElements = _CGetFieldAttribute(instance._CollectionID,
            instance._ModID, instance._RecordID, self._FieldID, 0, 0, 0, 0, 0,
            0, 1)
        oElements = [self._Type(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, x) for x in
            range(0, numElements)]
    ...
    def __set__(self, instance, nElements):
        if nElements is None or not len(nElements):
            _CDeleteField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0)
        else:
            length = len(nElements)
            if isinstance(nElements[0], tuple):
                nValues = nElements
            else:
                nValues = ExtractCopyList(nElements)
            ##Resizes the list
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0, 0,
                c_long(length))
            numElements = _CGetFieldAttribute(instance._CollectionID,
                instance._ModID, instance._RecordID, self._FieldID, 0, 0, 0, 0,
                0, 0, 1)
            oElements = [self._Type(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, x) for x in
                range(0, numElements)]
...
class CBashSINT32_OR_UNKNOWN(object):
    ...
    def __get__(self, instance, owner):
        type = _CGetFieldAttribute(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0, 2)
...
class CBashXSED(object):
    ...
    def __set__(self, instance, nValue):
        ...
        if nValue is None:
            ...
            if self._AsOffset:
                nValue = c_ubyte(nValue)
            else:
                nValue = c_ulong(nValue)
            # Borrowing ArraySize to flag if the new value is an offset
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0,
                byref(nValue), c_bool(self._AsOffset))
    ...
    def __get__(self, instance, owner):
        ...
        if (numRecords > 0):
            ...
            _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0,
                byref(cRecords))
            return [ISTRING(string_at(cRecords[x])) for x in
                range(0, numRecords)]
        return []
...
class CBashFORMID(object):
    ...
    def __get__(self, instance, owner):
        ...
        retValue = _CGetField(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0, 0)
        if (retValue): return MakeLongFid(instance._CollectionID,
            instance._ModID, retValue.contents.value)
        return None
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0,
                byref(c_ulong(MakeShortFid(instance._CollectionID, nValue))), 0)
...
class CBashMGEFCODE(object):
    ...
    def __get__(self, instance, owner):
        ...
        if (retValue):
            ...
            if type == API_FIELDS.STATIC_MGEFCODE:
                ...
                return MakeLongMGEFCode(instance._CollectionID, instance._ModID,
                    retValue.contents.value)
        return None
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0, byref(
                    c_ulong(MakeShortMGEFCode(instance._CollectionID,
                        instance._ModID, nValue))), 0)
    ...
    def __get__(self, instance, owner):
        ...
        if (numRecords > 0):
            ...
            _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0,
                byref(cRecords))
            return [MakeLongFid(instance._CollectionID, instance._ModID,
                cRecords.contents[x]) for x in range(0, numRecords)]
        return []
    ...
    def __get__(self, instance, owner):
        ...
        if (numRecords > 0):
            ...
            for x in range(numRecords):
                type = _CGetFieldAttribute(instance._CollectionID,
                    instance._ModID, instance._RecordID, self._FieldID, x, 1, 0,
                    0, 0, 0, 2)
                if type == API_FIELDS.UINT32:
                    values.append(cRecords[x])
                elif type == API_FIELDS.FORMID:
                    values.append(
                        MakeLongFid(instance._CollectionID, instance._ModID,
                            cRecords[x]))
        return values
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            for x, value in enumerate(nValue):
                ...
                _CSetField(instance._CollectionID, instance._ModID,
                    instance._RecordID, self._FieldID, x, 1, 0, 0, 0, 0,
                    byref(c_ulong(MakeShortFid(instance._CollectionID, value))),
                    IsFormID)
...
class CBashMGEFCODE_OR_UINT32_ARRAY(object):
        ...
        if (numRecords > 0):
            ...
            for x in range(numRecords):
                ...
                if type in (API_FIELDS.UINT32, API_FIELDS.STATIC_MGEFCODE):
                    ...
                    values.append(MakeLongMGEFCode(instance._CollectionID,
                        instance._ModID, cRecords.contents[x]))
        return values
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            nValue = [
                MakeShortMGEFCode(instance._CollectionID, instance._ModID, x)
                for x in nValue]
            cRecords = (c_ulong * length)(*nValue)
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0,
...
class CBashRECORDARRAY(object):
    ...
    def __get__(self, instance, owner):
        numRecords = _CGetNumRecords(instance._CollectionID, instance._ModID,
            self._TypeName)
        if (numRecords > 0):
            cRecords = (c_ulong * numRecords)()
            _CGetRecordIDs(instance._CollectionID, instance._ModID,
                self._TypeName, byref(cRecords))
            return [self._Type(instance._CollectionID, instance._ModID, x, 0,
                self._CopyFlags) for x in cRecords]
        return []
...
class CBashSUBRECORD(object):
    ...
    def __get__(self, instance, owner):
        ...
        retValue = _CGetField(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0, 0)
        if (retValue): return self._Type(instance._CollectionID,
            instance._ModID, retValue, instance._RecordID, self._CopyFlags)
        return None
    ...
    def __set__(self, instance, nValue):
        ...
        retValue = _CGetField(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0, 0)
        if (retValue):
            oSubRecord = self._Type(instance._CollectionID, instance._ModID,
                retValue, instance._RecordID, self._CopyFlags)
        else:
            oSubRecord = None
        if nSubRecord is None and oSubRecord is not None:
            oSubRecord.DeleteRecord()
        else:
            if oSubRecord is None:
                RecordID = _CCreateRecord(instance._CollectionID,
                    instance._ModID, self._TypeName, 0, 0, instance._RecordID,
                    c_ulong(self._CopyFlags))
                if (RecordID): oSubRecord = self._Type(instance._CollectionID,
                    instance._ModID, RecordID, instance._RecordID,
                    self._CopyFlags)
            if oSubRecord is None:
                return
    ...
    def __get__(self, instance, owner):
        ...
        if (numRecords > 0):
            ...
            _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, 0, 0, 0, 0, 0, 0,
                byref(cRecords))
            return [self._Type(instance._CollectionID, instance._ModID, x,
                instance._RecordID, self._CopyFlags) for x in cRecords]
        return []
...
class CBashLIST_LIST(object):
    ...
    def __get__(self, instance, owner):
        numElements = _CGetFieldAttribute(instance._CollectionID,
            instance._ModID, instance._RecordID, instance._FieldID,
            instance._ListIndex, self._ListFieldID, 0, 0, 0, 0, 1)
        oElements = [self._Type(instance._CollectionID, instance._ModID,
            instance._RecordID, instance._FieldID, instance._ListIndex,
            self._ListFieldID, x) for x in range(0, numElements)]
        if (self._AsList): return ExtractCopyList(oElements)
        return oElements
    ...
    def __set__(self, instance, nElements):
        if nElements is None or not len(nElements):
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, 0, c_long(length))
            numElements = _CGetFieldAttribute(instance._CollectionID,
                instance._ModID, instance._RecordID, instance._FieldID,
                instance._ListIndex, self._ListFieldID, 0, 0, 0, 0, 1)
            oElements = [self._Type(instance._CollectionID, instance._ModID,
...
class CBashGeneric_LIST(object):
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, byref(self._Type(nValue)), 0)
...
class CBashFORMIDARRAY_LIST(object):
    ...
    def __get__(self, instance, owner):
        ...
        if (numRecords > 0):
            ...
            _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, byref(cRecords))
            return [MakeLongFid(instance._CollectionID, instance._ModID,
                cRecords.contents[x]) for x in range(0, numRecords)]
        return []
    ...
    def __set__(self, instance, nValue):
        if nValue is None or not len(nValue):
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, byref(cRecords), length)
    ...
    def __set__(self, instance, nValue):
        ...
        if type != API_FIELDS.UNKNOWN:
            if nValue is None:
                ...
                _CSetField(instance._CollectionID, instance._ModID,
                    instance._RecordID, instance._FieldID, instance._ListIndex,
                    self._ListFieldID, 0, 0, 0, 0, byref(
                        c_ulong(MakeShortFid(instance._CollectionID, nValue))),
                    0)
...
class CBashMGEFCODE_OR_UINT32_LIST(object):
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, byref(c_ulong(
                    MakeShortMGEFCode(instance._CollectionID, instance._ModID,
                        nValue))), 0)
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            type = _CGetFieldAttribute(instance._CollectionID, instance._ModID,
                ...
                self._ListFieldID, 0, 0, 0, 0, 2)
            if type in (
            API_FIELDS.STATIC_MGEFCODE, API_FIELDS.RESOLVED_MGEFCODE):
                nValue = MakeShortMGEFCode(instance._CollectionID,
                    instance._ModID, nValue)
            elif type in (API_FIELDS.FORMID, API_FIELDS.STATIC_ACTORVALUE,
            API_FIELDS.RESOLVED_ACTORVALUE):
                nValue = MakeShortFid(instance._CollectionID, nValue)
            elif type == API_FIELDS.UINT32 and isinstance(nValue, tuple):
                return
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, byref(c_ulong(nValue)), 0)
...
class CBashFLOAT32_LIST(object):
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, byref(c_float(round(nValue, 6))),
                0)
...
class CBashSTRING_LIST(object):
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, 0, 0, 0, 0, str(nValue), 0)
...
class CBashLIST_LISTX2(object):
    ...
    def __get__(self, instance, owner):
        numElements = _CGetFieldAttribute(instance._CollectionID,
            instance._ModID, instance._RecordID, self._FieldID,
            instance._ListIndex, self._ListFieldID, instance._ListX2Index,
            self._ListX2FieldID, 0, 0, 1)
        oElements = [self._Type(instance._CollectionID, instance._ModID,
            instance._RecordID, instance._FieldID, instance._ListIndex,
            self._ListFieldID, instance._ListX2Index, self._ListX2FieldID, x)
    ...
    def __set__(self, instance, nElements):
        if nElements is None or not len(nElements):
            _CDeleteField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, instance._ListIndex,
                self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
                0, 0)
        else:
            length = len(nElements)
            if isinstance(nElements[0], tuple):
            ...
            oElements = [self._Type(instance._CollectionID, instance._ModID,
                instance._RecordID, instance._FieldID, instance._ListIndex,
                self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
                x) for x in range(0, numElements)]
            SetCopyList(oElements, nValues)
...
class CBashGeneric_LISTX2(object):
    ...
    def __get__(self, instance, owner):
        ...
        retValue = _CGetField(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, instance._ListIndex,
            self._ListFieldID, instance._ListX2Index, self._ListX2FieldID, 0, 0,
            0)
        if (retValue): return retValue.contents.value
        return None
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, instance._ListIndex,
                self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
                0, 0, byref(c_float(round(nValue, 6))), 0)
    ...
    def __get__(self, instance, owner):
        ...
        if (numRecords > 0):
            ...
            for x in range(numRecords):
                type = _CGetFieldAttribute(instance._CollectionID,
                    instance._ModID, instance._RecordID, self._FieldID,
                    instance._ListIndex, self._ListFieldID,
                    instance._ListX2Index, self._ListX2FieldID, x, 1, 2)
                if type == API_FIELDS.UINT32:
                    values.append(cRecords[x])
                elif type == API_FIELDS.FORMID:
                    values.append(
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            ...
            for x, value in enumerate(nValue):
                ...
                _CSetField(instance._CollectionID, instance._ModID,
                    instance._RecordID, self._FieldID, instance._ListIndex,
                    self._ListFieldID, instance._ListX2Index,
                    self._ListX2FieldID, x, 1,
                    byref(c_ulong(MakeShortFid(instance._CollectionID, value))),
                    IsFormID)
        ...
        if type != API_FIELDS.UNKNOWN:
            if nValue is None:
                _CDeleteField(instance._CollectionID, instance._ModID,
                    instance._RecordID, self._FieldID, instance._ListIndex,
                    self._ListFieldID, instance._ListX2Index,
                    self._ListX2FieldID, 0, 0)
            else:
                _CSetField(instance._CollectionID, instance._ModID,
                    instance._RecordID, self._FieldID, instance._ListIndex,
                    self._ListFieldID, instance._ListX2Index,
                    self._ListX2FieldID, 0, 0, byref(
                        c_ulong(MakeShortFid(instance._CollectionID, nValue))),
                    0)
...
class CBashGeneric_LISTX3(object):
    ...
    def __get__(self, instance, owner):
        ...
        retValue = _CGetField(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, instance._ListIndex,
            self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
            instance._ListX3Index, self._ListX3FieldID, 0)
        if (retValue): return retValue.contents.value
        return None
    ...
    def __set__(self, instance, nValue):
        if nValue is None:
            _CDeleteField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, instance._ListIndex,
                self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
                instance._ListX3Index, self._ListX3FieldID)
        else:
            _CSetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, instance._ListIndex,
                self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
...
class CBashUINT8ARRAY_LISTX3(object):
    ...
    def __get__(self, instance, owner):
        numRecords = _CGetFieldAttribute(instance._CollectionID,
            instance._ModID, instance._RecordID, self._FieldID,
            instance._ListIndex, self._ListFieldID, instance._ListX2Index,
            self._ListX2FieldID, instance._ListX3Index, self._ListX3FieldID, 1)
        if (numRecords > 0):
            cRecords = POINTER(c_ubyte * numRecords)()
            _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, instance._ListIndex,
                self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
                instance._ListX3Index, self._ListX3FieldID, byref(cRecords))
            return [cRecords.contents[x] for x in range(0, numRecords)]
        return []
...
class CBashUNKNOWN_OR_FORMID_OR_UINT32_LISTX3(object):
    ...
    def __get__(self, instance, owner):
        type = _CGetFieldAttribute(instance._CollectionID, instance._ModID,
            instance._RecordID, self._FieldID, instance._ListIndex,
            self._ListFieldID, instance._ListX2Index, self._ListX2FieldID,
            instance._ListX3Index, self._ListX3FieldID, 2)
        if type != API_FIELDS.UNKNOWN:
            _CGetField.restype = POINTER(c_long)
            retValue = _CGetField(instance._CollectionID, instance._ModID,
                instance._RecordID, self._FieldID, instance._ListIndex,
        ...
        if type != API_FIELDS.UNKNOWN:
            if nValue is None:
                _CDeleteField(instance._CollectionID, instance._ModID,
                    instance._RecordID, self._FieldID, instance._ListIndex,
                    self._ListFieldID, instance._ListX2Index,
                    self._ListX2FieldID, instance._ListX3Index,
                    self._ListX3FieldID)
            else:
                _CSetField(instance._CollectionID, instance._ModID,
                    instance._RecordID, self._FieldID, instance._ListIndex,
...
class ListX3Component(object):
    def __init__(self, CollectionIndex, ModID, RecordID, FieldID, ListIndex,
        ListFieldID, ListX2Index, ListX2FieldID, ListX3Index):
        self._CollectionID = CollectionIndex
        self._ModID = ModID
        self._RecordID = RecordID
        self._FieldID = FieldID
...
class Effect(ListComponent):
    ...
    IsUsingRecoversOverride = CBashBasicFlag('efixOverrides',
        0x00000002)  # OBME
    IsUsingParamFlagAOverride = CBashBasicFlag('efixOverrides',
        0x00000004)  # OBME
    IsUsingBeneficialOverride = CBashBasicFlag('efixOverrides',
        0x00000008)  # OBME
    IsUsingEFIXParamOverride = CBashBasicFlag('efixOverrides',
        0x00000010)  # OBME
    ...
    IsUsingBaseCostOverride = CBashBasicFlag('efixOverrides',
        0x00000100)  # OBME
    IsUsingResistAVOverride = CBashBasicFlag('efixOverrides',
        0x00000200)  # OBME
    IsUsingFXPersistsOverride = CBashBasicFlag('efixOverrides',
        0x00000400)  # OBME
    IsUsingIconOverride = CBashBasicFlag('efixOverrides', 0x00000800)  # OBME
    IsUsingDoesntTeachOverride = CBashBasicFlag('efixOverrides',
        0x00001000)  # OBME
    IsUsingUnknownFOverride = CBashBasicFlag('efixOverrides',
        0x00004000)  # OBME
    IsUsingNoRecastOverride = CBashBasicFlag('efixOverrides',
        0x00008000)  # OBME
    IsUsingParamFlagBOverride = CBashBasicFlag('efixOverrides',
        0x00010000)  # OBME
    IsUsingMagnitudeIsRangeOverride = CBashBasicFlag('efixOverrides',
        0x00020000)  # OBME
    IsUsingAtomicResistanceOverride = CBashBasicFlag('efixOverrides',
        0x00040000)  # OBME
    IsUsingParamFlagCOverride = CBashBasicFlag('efixOverrides',
        0x00080000)  # OBME
    IsUsingParamFlagDOverride = CBashBasicFlag('efixOverrides',
        0x00100000)  # OBME
    IsUsingDisabledOverride = CBashBasicFlag('efixOverrides',
        0x00400000)  # OBME
    IsUsingUnknownOOverride = CBashBasicFlag('efixOverrides',
        0x00800000)  # OBME
    IsUsingNoHitEffectOverride = CBashBasicFlag('efixOverrides',
        0x08000000)  # OBME
    IsUsingPersistOnDeathOverride = CBashBasicFlag('efixOverrides',
        0x10000000)  # OBME
    IsUsingExplodesWithForceOverride = CBashBasicFlag('efixOverrides',
        0x20000000)  # OBME
    ...
    IsExplodesWithForceOverride = CBashBasicFlag('efixFlags',
        0x20000000)  # OBME
    IsHiddenOverride = CBashBasicFlag('efixFlags', 0x40000000)  # OBME
    exportattrs = copyattrs = ['name', 'magnitude', 'area', 'duration',
        'rangeType',
...
class ObBaseRecord(object):
    ...
    def __init__(self, CollectionIndex, ModID, RecordID, ParentID=0,
        CopyFlags=0):
        self._CollectionID = CollectionIndex
        self._ModID = ModID
        self._RecordID = RecordID
    ...
    def IsIdenticalTo(self, other):
        if type(other) is type(self):
            ...
            a = other.flags2
            try:
                identical = _CCompareIdenticalRecords(self._RecordID,
                    other._RecordID)
                ...
                deprint(_("Exception reading fid: %s from mod: %s %s" % (
                str(self.fid), self.ModName, err)))
                ...
                if self._Type == 'CELL':
                    ...
                    for attr in ('PGRD', 'LAND'):
                        if getattr(self, attr):
                            if getattr(other, attr):
                                if not getattr(self, attr).IsIdenticalTo(
                                    getattr(other, attr)):
                                    return False
                            else:
                                return False
                    ...
                    if self.Parent:
                        if other.Parent:
                            ...
                            return False
                        if (self.Parent.posX, self.Parent.posY) != (
                        other.Parent.posX, other.Parent.posY):
                            return False
    ...
    def GetNumReferences(self, FormIDToMatch):
        ...
        return _CGetNumReferences(self._CollectionID, self._ModID,
            self._RecordID, FormIDToMatch)
    ...
    def UpdateReferences(self, FormIDToReplace, ReplacementFormID):
        ...
        return _CUpdateReferences(self._CollectionID, self._ModID,
            self._RecordID, FormIDToReplace, ReplacementFormID)

    def History(self):
        cModIDs = (
        c_ulong * 257)()  # just allocate enough for the max number + size
        cRecordIDs = (
        c_ulong * 257)()  # just allocate enough for the max number + size
        numRecords = _CGetRecordHistory(self._CollectionID, self._ModID,
            self._RecordID, byref(cModIDs), byref(cRecordIDs))
        parent = getattr(self, '_ParentID', 0)
        return [self.__class__(self._CollectionID, cModIDs[x], cRecordIDs[x],
            parent, self._CopyFlags) for x in range(0, numRecords)]
    ...
    def IsWinning(self, GetExtendedConflicts=False):
        ...
        return _CIsRecordWinning(self._CollectionID, self._ModID,
            self._RecordID, c_ulong(GetExtendedConflicts))

    def Conflicts(self, GetExtendedConflicts=False):
        numRecords = _CGetNumRecordConflicts(self._CollectionID, self._RecordID,
            c_ulong(GetExtendedConflicts))  # gives upper bound
        if (numRecords > 1):
            cModIDs = (c_ulong * numRecords)()
            cRecordIDs = (c_ulong * numRecords)()
            numRecords = _CGetRecordConflicts(self._CollectionID,
                self._RecordID, byref(cModIDs), byref(cRecordIDs),
                c_ulong(GetExtendedConflicts))
            parent = getattr(self, '_ParentID', 0)
            return [
                self.__class__(self._CollectionID, cModIDs[x], cRecordIDs[x],
                    parent, self._CopyFlags) for x in range(0, numRecords)]
        return []
        ...
        if parentRecords:
            for attr in attrs:
                if isinstance(attr, (str, unicode)):
                    # Single attr
                    tempconflicting.update(
                        [(attr, reduce(getattr, attr.split('.'), self)) for
                            parentRecord in parentRecords if
                            reduce(getattr, attr.split('.'), self) != reduce(
                                getattr, attr.split('.'), parentRecord)])
                    ...
                    for parentRecord in parentRecords:
                        ...
                        conflict = False
                        for subattr in attr:
                            if not conflict and reduce(getattr,
                                subattr.split('.'), self) != reduce(getattr,
                                subattr.split('.'), parentRecord):
                                conflict = True
                            subconflicting.update([(subattr,
                            reduce(getattr, subattr.split('.'), self))])
                        if conflict:
            ...
            for attr in attrs:
                if isinstance(attr, (str, unicode)):
                    tempconflicting.update(
                        [(attr, reduce(getattr, attr.split('.'), self))])
                elif isinstance(attr, (list, tuple, set)):
                    tempconflicting.update(
                        [(subattr, reduce(getattr, subattr.split('.'), self))
                            for subattr in attr])
        if self._Type in FORMIDAttrs:
            curformidattrs = FORMIDAttrs[self._Type]
            for attr in tempconflicting:
                if attr in curformidattrs:
                    if isinstance(tempconflicting[attr], tuple) and \
                            tempconflicting[attr][0] == None:
                        try:
                            deprint(_(
                                "%s attribute of %s record (maybe named: %s) importing from %s referenced an unloaded object (probably %s) - value skipped") % (
                                    attr, self.fid, self.full, self.GName,
                                    tempconflicting[attr]))
                        except:  # a record type that doesn't have a full subrecord:
                            deprint(_(
                                "%s attribute of %s record importing from %s referenced an unloaded object (probably %s) - value skipped") % (
                                    attr, self.fid, self.GName,
                                    tempconflicting[attr]))
                        continue
                conflicting[attr] = tempconflicting[attr]
        else:
            conflicting = tempconflicting
        ##        if parentRecords:
        ##            for parentRecord in parentRecords:
        ##                for attr in attrs:
    ...
    def CopyAsOverride(self, target, CopyFlags=None):
        ...
        if hasattr(self, '_ParentID'):
            if isinstance(target, ObModFile):
                targetID = self._ParentID
            else:
                targetID = target._RecordID
        ...
        RecordID = _CCopyRecord(self._CollectionID, self._ModID, self._RecordID,
            target._ModID, targetID, 0, 0, c_ulong(CopyFlags))
        # Clear SetAsOverride
        CopyFlags &= ~0x00000001
        if (RecordID): return self.__class__(self._CollectionID, target._ModID,
            RecordID, getattr(self, '_ParentID', 0), CopyFlags)
        return None
    ...
    def CopyAsNew(self, target, RecordFormID=0, CopyFlags=None):
        ...
        CopyFlags &= ~0x00000001
        RecordID = _CCopyRecord(self._CollectionID, self._ModID, self._RecordID,
            target._ModID, targetID, RecordFormID, 0, c_ulong(CopyFlags))
        if (RecordID): return self.__class__(self._CollectionID, target._ModID,
            RecordID, getattr(self, '_ParentID', 0), CopyFlags)
        return None
    ...
    def Parent(self):
        ...
        if ParentID:
            testRecord = ObBaseRecord(self._CollectionID, self._ModID, ParentID,
                0, 0)
            RecordType = type_record[testRecord.recType]
            if RecordType:
                return RecordType(self._CollectionID, self._ModID, ParentID, 0,
                    0)
        return None
    ...
    def recType(self):
        ...
        retValue = _CGetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 0, 0, 0, 0, 0, 0, 0, 0)
        _CGetFieldAttribute.restype = c_ulong
    ...
    def get_fid(self):
        _CGetField.restype = POINTER(c_ulong)
        retValue = _CGetField(self._CollectionID, self._ModID, self._RecordID,
            2, 0, 0, 0, 0, 0, 0, 0)
        if (retValue): return MakeLongFid(self._CollectionID, self._ModID,
            retValue.contents.value)
        return None
    ...
    def set_fid(self, nValue):
        if nValue is None:
            ...
            nValue = MakeShortFid(self._CollectionID, nValue)
        _CSetRecordIdentifiers(self._CollectionID, self._ModID, self._RecordID,
            nValue, self.eid or 0)
    ...
    def set_eid(self, nValue):
        if nValue is None or not len(nValue):
            ...
            nValue = str(nValue)
        _CSetRecordIdentifiers(self._CollectionID, self._ModID, self._RecordID,
            MakeShortFid(self._CollectionID, self.fid or 0), nValue)
...
class ObTES4Record(object):
    ...
    exportattrs = copyattrs = ['flags1', 'flags2', 'version', 'numRecords',
        'nextObject',
...
class ObGMSTRecord(ObBaseRecord):
    ...
    def get_value(self):
        rFormat = _CGetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 5, 0, 0, 0, 0, 0, 0, 2)
        if (rFormat == API_FIELDS.UNKNOWN):
            return None
        elif (rFormat == API_FIELDS.SINT32):
            _CGetField.restype = POINTER(c_long)
            retValue = _CGetField(self._CollectionID, self._ModID,
                self._RecordID, 5, 0, 0, 0, 0, 0, 0, 0)
            if (retValue): return retValue.contents.value
            ...
            _CGetField.restype = c_char_p
            return _CGetField(self._CollectionID, self._ModID, self._RecordID,
                5, 0, 0, 0, 0, 0, 0, 0)
        return None

    def set_value(self, nValue):
        if nValue is None:
            _CDeleteField(self._CollectionID, self._ModID, self._RecordID, 5, 0,
                0, 0, 0, 0, 0)
        else:
            rFormat = _CGetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 5, 0, 0, 0, 0, 0, 0, 2)
            if (rFormat == API_FIELDS.SINT32 and type(nValue) is int):
                _CSetField(self._CollectionID, self._ModID, self._RecordID, 5,
                    0, 0, 0, 0, 0, 0, byref(c_long(nValue)), 0)
            elif (rFormat == API_FIELDS.FLOAT32 == 10 and type(
                nValue) is float):
                _CSetField(self._CollectionID, self._ModID, self._RecordID, 5,
                    0, 0, 0, 0, 0, 0, byref(c_float(round(nValue, 6))), 0)
            elif (rFormat == API_FIELDS.STRING and type(nValue) is str):
                _CSetField(self._CollectionID, self._ModID, self._RecordID, 5,
                    0, 0, 0, 0, 0, 0, nValue, 0)
...
class ObACHRRecord(ObBaseRecord):
    def __init__(self, CollectionIndex, ModID, RecordID, ParentID=0,
        CopyFlags=0):
        ObBaseRecord.__init__(self, CollectionIndex, ModID, RecordID, ParentID,
            CopyFlags)
    ...
    copyattrs = ObBaseRecord.baseattrs + ['base', 'unknownXPCIFormID',
        'unknownXPCIString',
        'lod1', 'lod2', 'lod3', 'parent', 'parentFlags',
        'merchantContainer', 'horse', 'xrgd_p', 'scale',
        'posX', 'posY', 'posZ', 'rotX', 'rotY', 'rotZ']
    exportattrs = ObBaseRecord.baseattrs + ['base', 'unknownXPCIFormID',
        'unknownXPCIString',
        'lod1', 'lod2', 'lod3', 'parent', 'parentFlags',
...
class ObACRERecord(ObBaseRecord):
    ...
    IsOppositeParent = CBashBasicFlag('parentFlags', 0x00000001)
    copyattrs = ObBaseRecord.baseattrs + ['base', 'owner', 'rank',
        'globalVariable',
        'lod1', 'lod2', 'lod3', 'parent', 'parentFlags',
        'xrgd_p', 'scale', 'posX', 'posY', 'posZ', 'rotX',
        'rotY', 'rotZ']
    exportattrs = ObBaseRecord.baseattrs + ['base', 'owner', 'rank',
        'globalVariable',
        'lod1', 'lod2', 'lod3', 'parent', 'parentFlags',
...
class ObINFORecord(ObBaseRecord):
    ...
    def create_response(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 11, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Response(self._CollectionID, self._ModID, self._RecordID,
            11, length)
    ...
    def create_condition(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 12, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Condition(self._CollectionID, self._ModID, self._RecordID, 12,
            length)
    ...
    copyattrs = ObBaseRecord.baseattrs + ['dialType', 'flags', 'quest', 'topic',
        ...
        'references']
    exportattrs = ObBaseRecord.baseattrs + ['dialType', 'flags', 'quest',
        'topic',
        'prevInfo', 'addTopics', 'responses_list',
        'conditions_list', 'choices', 'linksFrom',
        'numRefs', 'compiledSize', 'lastIndex',
...
class ObLANDRecord(ObBaseRecord):
    ...
    class AlphaLayer(ListComponent):
        ...
        def create_opacity(self):
            length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 12, self._ListIndex, 5, 0, 0, 0, 0, 1)
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12,
                self._ListIndex, 5, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Opacity(self._CollectionID, self._ModID, self._RecordID,
                12, self._ListIndex, 5, length)
    ...
    def get_normals(self):
        return [[
            self.Normal(self._CollectionID, self._ModID, self._RecordID, 6, x,
                0, y) for y in range(0, 33)] for x in range(0, 33)]
    ...
    def get_heights(self):
        return [[
            self.Height(self._CollectionID, self._ModID, self._RecordID, 8, x,
                0, y) for y in range(0, 33)] for x in range(0, 33)]
    ...
    def get_colors(self):
        return [[
            self.Color(self._CollectionID, self._ModID, self._RecordID, 10, x,
                0, y) for y in range(0, 33)] for x in range(0, 33)]
    ...
    def create_baseTextures(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.BaseTexture(self._CollectionID, self._ModID, self._RecordID,
            11, length)
    ...
    def create_alphaLayer(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.AlphaLayer(self._CollectionID, self._ModID, self._RecordID,
            12, length)
    ...
    def create_vertexTextures(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 13, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 13, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.VertexTexture(self._CollectionID, self._ModID,
            self._RecordID, 13, length)
    ...
    def get_Positions(self):
        return [[
            self.Position(self._CollectionID, self._ModID, self._RecordID, 14,
                row, 0, column) for column in range(0, 33)] for row in
            range(0, 33)]
    ...
    copyattrs = ObBaseRecord.baseattrs + ['data_p', 'normals_list',
        'heights_list', 'heightOffset',
        'colors_list', 'baseTextures_list', 'alphaLayers_list',
        'vertexTextures_list']
    exportattrs = ObBaseRecord.baseattrs + ['normals_list', 'heights_list',
        'heightOffset',
        'colors_list', 'baseTextures_list', 'alphaLayers_list',
...
class ObPGRDRecord(ObBaseRecord):
    ...
    def create_pgrp(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 6, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 6, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
    ...
    def create_pgri(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 9, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 9, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.PGRI(self._CollectionID, self._ModID, self._RecordID, 9,
            length)
    ...
    def create_pgrl(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 10, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 10, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.PGRL(self._CollectionID, self._ModID, self._RecordID, 10,
            length)

    pgrl = CBashLIST(10, PGRL)
    pgrl_list = CBashLIST(10, PGRL, True)

    copyattrs = ObBaseRecord.baseattrs + ['count', 'pgrp_list', 'pgag_p',
        'pgrr_p',
        'pgri_list', 'pgrl_list']
...
class ObROADRecord(ObBaseRecord):
    ...
    def create_pgrp(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 5, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 5, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
    ...
    def create_pgrr(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 6, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.PGRR(self._CollectionID, self._ModID, self._RecordID, 6,
            length)

    pgrr = CBashLIST(6, PGRR)
    pgrr_list = CBashLIST(6, PGRR, True)

    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['pgrp_list',
        'pgrr_list']
...
class ObACTIRecord(ObBaseRecord):
    ...
    copyattrs = ObBaseRecord.baseattrs + ['full', 'modPath', 'modb', 'modt_p',
        'script',
        'sound']
    exportattrs = ObBaseRecord.baseattrs + ['full', 'modPath', 'modb', 'script',
        'sound']  # 'modt_p',
...
class ObALCHRecord(ObBaseRecord):
    ...
    def create_effect(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 15, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 15, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Effect(self._CollectionID, self._ModID, self._RecordID, 15,
            length)
...
class ObANIORecord(ObBaseRecord):
    ...
    copyattrs = ObBaseRecord.baseattrs + ['modPath', 'modb', 'modt_p',
        'animationId']
    exportattrs = ObBaseRecord.baseattrs + ['modPath', 'modb',
        'animationId']  # 'modt_p',
...
class ObARMORecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'script',
...
class ObBSGNRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'iconPath',
        'text', 'spells']
...
class ObCELLRecord(ObBaseRecord):
    ...
    def create_ACHR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ACHR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObACHRRecord(self._CollectionID, self._ModID,
    ...
    def create_ACRE(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ACRE", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObACRERecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 0)
        return None
    ...
    def create_REFR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("REFR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObREFRRecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 0)
        return None
    ...
    def create_PGRD(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("PGRD", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObPGRDRecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 0)
        return None
    ...
    def create_LAND(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("LAND", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObLANDRecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 0)
        return None
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'flags',
...
class ObCLASRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'description',
        'iconPath', 'primary1',
...
class ObCLMTRecord(ObBaseRecord):
    ...
    def create_weather(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 5, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Weather(self._CollectionID, self._ModID, self._RecordID, 5,
            length)
    ...
    copyattrs = ObBaseRecord.baseattrs + ['weathers_list', 'sunPath',
        'glarePath', 'modPath',
        'modb', 'modt_p', 'riseBegin', 'riseEnd',
        'setBegin', 'setEnd', 'volatility', 'phaseLength']
    exportattrs = ObBaseRecord.baseattrs + ['weathers_list', 'sunPath',
        'glarePath', 'modPath',
...
class ObCREARecord(ObBaseRecord):
    ...
    def create_faction(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 19, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 19, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Faction(self._CollectionID, self._ModID, self._RecordID, 19,
            length)
    ...
    def create_item(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 22, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 22, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
    ...
    def create_sound(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 58, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 58, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Sound(self._CollectionID, self._ModID, self._RecordID, 58,
            length)
...
class ObCSTYRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['dodgeChance',
        'lrChance', 'lrTimerMin', 'lrTimerMax',
...
class ObDIALRecord(ObBaseRecord):
    ...
    def create_INFO(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("INFO", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObINFORecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 0)
        return None
    ...
    IsService = CBashBasicType('dialType', 5, 'IsTopic')
    IsMisc = CBashBasicType('dialType', 6, 'IsTopic')
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['quests',
        'removedQuests',
        'full', 'dialType']
...
class ObEFSHRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['fillTexturePath',
        'particleTexturePath', 'flags', 'memSBlend', 'memBlendOp',
        'memZFunc', 'fillRed', 'fillGreen', 'fillBlue', 'fillAIn', 'fillAFull',
        'fillAOut', 'fillAPRatio', 'fillAAmp', 'fillAFreq', 'fillAnimSpdU',
        'fillAnimSpdV', 'edgeOff', 'edgeRed', 'edgeGreen', 'edgeBlue',
        'edgeAIn',
...
class ObENCHRecord(ObBaseRecord):
    ...
    def create_effect(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Effect(self._CollectionID, self._ModID, self._RecordID, 11,
            length)
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'itemType',
        'chargeAmount',
...
class ObFACTRecord(ObBaseRecord):
    ...
    def create_relation(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 6, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return Relation(self._CollectionID, self._ModID, self._RecordID, 6,
            length)
    ...
    def create_rank(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 9, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Rank(self._CollectionID, self._ModID, self._RecordID, 9,
            length)
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full',
        'relations_list', 'flags',
...
class ObIDLERecord(ObBaseRecord):
    ...
    def create_condition(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 8, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 8, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return Condition(self._CollectionID, self._ModID, self._RecordID, 8,
            length)
...
class ObLIGHRecord(ObBaseRecord):
    ...
    copyattrs = ObBaseRecord.baseattrs + ['modPath', 'modb', 'modt_p', 'script',
        'full',
...
class ObLSCRRecord(ObBaseRecord):
    ...
    def create_location(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 7, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 7, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Location(self._CollectionID, self._ModID, self._RecordID, 7,
            length)

    locations = CBashLIST(7, Location)
    locations_list = CBashLIST(7, Location, True)

    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['iconPath', 'text',
        'locations_list']
...
class ObLTEXRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['iconPath', 'flags',
...
class ObLVLCRecord(ObBaseRecord):
    ...
    def create_entry(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 9, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Entry(self._CollectionID, self._ModID, self._RecordID, 9,
            length)
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['chanceNone', 'flags',
        'script',
...
class ObLVLIRecord(ObBaseRecord):
    ...
    flags = CBashGeneric(6, c_ubyte)
    script = CBashJunk(
        7)  # Doesn't actually exist, but is here so that LVLC,LVLI,LVSP can be processed similarly
...
class ObMGEFRecord(ObBaseRecord):
    ...
    IsDetrimental = CBashBasicFlag('flags',
        0x00000004)  # OBME Deprecated, used for ValueModifier effects AV is decreased rather than increased
    IsMagnitudeIsPercent = CBashBasicFlag('flags',
        0x00000008)  # OBME Deprecated
    ...
    IsUnknownF = CBashBasicFlag('flags',
        0x00004000)  # no effects have this flag set
    IsNoRecast = CBashBasicFlag('flags',
        0x00008000)  # no effects have this flag set
    ...
    IsPCHasEffect = CBashBasicFlag('flags',
        0x00200000)  # whether or not PC has effect, forced to zero during loading
    IsDisabled = CBashBasicFlag('flags',
        0x00400000)  # Changeable, many if not all methods that loop over effects ignore those with this flag.
    #  Spells with an effect with this flag are apparently uncastable.
    IsUnknownO = CBashBasicFlag('flags',
        0x00800000)  # Changeable, POSN,DISE - these effects have *only* this bit set,
    #  perhaps a flag for meta effects
    IsUseAV = CBashBasicFlag('flags',
        0x01000000)  # OBME Deprecated, Changeable, but once set by default or by a previously loaded mod file
    #  it cannot be unset by another mod, nor can the mgefParam be overriden

    IsBallType = CBashMaskedType('flags', 0x06000000, 0,
        'IsBoltType')  # Changeable
    IsFogType = CBashMaskedType('flags', 0x06000000, 0x06000000,
        'IsBallType')  # Changeable

    def get_IsSprayType(self):
        return self.flags != None and not self.IsFogType and (
                                                             self.flags & 0x02000000) != 0

    def set_IsSprayType(self, nValue):
        if nValue:
            self.flags &= ~0x06000000
            self.flags |= 0x02000000
        elif self.IsSprayType:
    ...
    def get_IsBoltType(self):
        return self.flags != None and not self.IsFogType and (
                                                             self.flags & 0x04000000) != 0

    def set_IsBoltType(self, nValue):
        if nValue:
            self.flags &= ~0x06000000
            self.flags |= 0x04000000
        elif self.IsBoltType:
            self.IsBallType = True

    IsBoltType = property(get_IsBoltType, set_IsBoltType)  # Changeable

    IsFogType = CBashBasicFlag('flags', 0x06000000)  # Changeable
    IsNoHitEffect = CBashBasicFlag('flags',
        0x08000000)  # Changeable, no hit shader on target
    IsPersistOnDeath = CBashBasicFlag('flags',
        0x10000000)  # Effect is not automatically removed when its target dies
    IsExplodesWithForce = CBashBasicFlag('flags',
        0x20000000)  # causes explosion that can move loose objects (e.g. ragdolls)
    ...
    IsParamFlagA = CBashBasicFlag('OBMEFlags',
        0x00000004)  # OBME //Meaning varies with effect handler
    IsParamFlagB = CBashBasicFlag('OBMEFlags',
        0x00010000)  # OBME //Meaning varies with effect handler
    IsParamFlagC = CBashBasicFlag('OBMEFlags',
        0x00080000)  # OBME //Meaning varies with effect handler
    IsParamFlagD = CBashBasicFlag('OBMEFlags',
        0x00100000)  # OBME //Meaning varies with effect handler
    ...
    copyattrs = ObBaseRecord.baseattrs + ['full', 'text', 'iconPath', 'modPath',
        ...
        'counterEffects']
    exportattrs = ObBaseRecord.baseattrs + ['full', 'text', 'iconPath',
        'modPath',
        'modb', 'flags', 'baseCost',
        'associated', 'school', 'resistValue',
        'numCounters', 'light', 'projectileSpeed',
...
class ObNPC_Record(ObBaseRecord):
    ...
    def create_faction(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 16, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 16, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Faction(self._CollectionID, self._ModID, self._RecordID, 16,
            length)
    ...
    def create_item(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 21, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 21, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
...
class ObPACKRecord(ObBaseRecord):
    ...
    def create_condition(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 19, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Condition(self._CollectionID, self._ModID, self._RecordID, 19,
            length)
    ...
    IsLocNearCurrentLocation = CBashBasicType('locType', 2,
        'IsLocNearReference')
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['flags', 'aiType',
        'locType', 'locId',
...
class ObQUSTRecord(ObBaseRecord):
    ...
    class Stage(ListComponent):
        class Entry(ListX2Component):
            class ConditionX3(ListX3Component):
                ...
                IsNotEqual = CBashMaskedType('operType', 0xF0, 0x20, 'IsEqual')
                IsGreater = CBashMaskedType('operType', 0xF0, 0x40, 'IsEqual')
                IsGreaterOrEqual = CBashMaskedType('operType', 0xF0, 0x60,
                    'IsEqual')
                IsLess = CBashMaskedType('operType', 0xF0, 0x80, 'IsEqual')
                IsLessOrEqual = CBashMaskedType('operType', 0xF0, 0xA0,
                    'IsEqual')
                IsOr = CBashBasicFlag('operType', 0x01)
                IsRunOnTarget = CBashBasicFlag('operType', 0x02)
                IsUseGlobal = CBashBasicFlag('operType', 0x04)
                exportattrs = copyattrs = ['operType', 'compValue', 'ifunc',
                    'param1',
                    'param2']
            ...
            def create_condition(self):
                length = CBash.GetFieldAttribute(self._CollectionID,
                    self._ModID, self._RecordID, 11, self._ListIndex, 2,
                    self._ListX2Index, 2, 0, 0, 1)
                CBash.SetField(self._CollectionID, self._ModID, self._RecordID,
                    11, self._ListIndex, 2, self._ListX2Index, 2, 0, 0, 0,
                    c_ulong(length + 1))
                return self.ConditionX3(self._CollectionID, self._ModID,
                    self._RecordID, 11, self._ListIndex, 2, self._ListX2Index,
                    2, length)
            ...
            copyattrs = ['flags', 'conditions_list', 'text', 'numRefs',
                'compiledSize',
                'lastIndex', 'scriptType', 'compiled_p', 'scriptText',
                'references']
            exportattrs = ['flags', 'conditions_list', 'text', 'numRefs',
                'compiledSize',
        ...
        def create_entry(self):
            length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 11, self._ListIndex, 2, 0, 0, 0, 0, 1)
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11,
                self._ListIndex, 2, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Entry(self._CollectionID, self._ModID, self._RecordID,
                11, self._ListIndex, 2, length)
    ...
    class Target(ListComponent):
        ...
        def create_condition(self):
            length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 12, self._ListIndex, 4, 0, 0, 0, 0, 1)
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12,
                self._ListIndex, 4, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.ConditionX2(self._CollectionID, self._ModID,
                self._RecordID, 12, self._ListIndex, 4, length)
    ...
    def create_condition(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 10, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return Condition(self._CollectionID, self._ModID, self._RecordID, 10,
            length)
    ...
    def create_stage(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Stage(self._CollectionID, self._ModID, self._RecordID, 11,
            length)
    ...
    def create_target(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Target(self._CollectionID, self._ModID, self._RecordID, 12,
            length)
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['script', 'full',
        'iconPath',
...
class ObRACERecord(ObBaseRecord):
    ...
    def create_relation(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 8, 0, 0,
            0, 0, 0, 0, 0, c_ulong(length + 1))
        return Relation(self._CollectionID, self._ModID, self._RecordID, 8,
            length)
...
class ObREGNRecord(ObBaseRecord):
    ...
    class Area(ListComponent):
        ...
        def create_point(self):
            ...
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11,
                self._ListIndex, 2, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Point(self._CollectionID, self._ModID, self._RecordID,
                11, self._ListIndex, 2, length)
    ...
    class Entry(ListComponent):
        class Object(ListX2Component):
            ...
            exportattrs = copyattrs = ['objectId', 'parentIndex', 'density',
                'clustering',
        ...
        def create_object(self):
            ...
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12,
                self._ListIndex, 5, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Object(self._CollectionID, self._ModID, self._RecordID,
                12, self._ListIndex, 5, length)
        ...
        def create_grass(self):
            length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 12, self._ListIndex, 8, 0, 0, 0, 0, 1)
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12,
                self._ListIndex, 8, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Grass(self._CollectionID, self._ModID, self._RecordID,
                12, self._ListIndex, 8, length)
        ...
        def create_sound(self):
            length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 12, self._ListIndex, 10, 0, 0, 0, 0, 1)
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12,
                self._ListIndex, 10, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Sound(self._CollectionID, self._ModID, self._RecordID,
                12, self._ListIndex, 10, length)
        ...
        def create_weather(self):
            length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
                self._RecordID, 12, self._ListIndex, 11, 0, 0, 0, 0, 1)
            CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12,
                self._ListIndex, 11, 0, 0, 0, 0, 0, c_ulong(length + 1))
            return self.Weather(self._CollectionID, self._ModID, self._RecordID,
                12, self._ListIndex, 11, length)
        ...
        exportattrs = copyattrs = ['entryType', 'flags', 'priority',
            'objects_list', 'mapName',
            'iconPath', 'grasses_list', 'musicType', 'sounds_list',
            'weathers_list']
    ...
    def create_area(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 11, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Area(self._CollectionID, self._ModID, self._RecordID, 11,
            length)
    ...
    def create_entry(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Entry(self._CollectionID, self._ModID, self._RecordID, 12,
            length)

    entries = CBashLIST(12, Entry)
    entries_list = CBashLIST(12, Entry, True)

    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['iconPath', 'mapRed',
        'mapGreen',
...
class ObSBSPRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['sizeX', 'sizeY',
        'sizeZ']
...
class ObSCPTRecord(ObBaseRecord):
    ...
    def create_var(self):
        ...
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 12, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Var(self._CollectionID, self._ModID, self._RecordID, 12,
            length)
    ...
    copyattrs = ObBaseRecord.baseattrs + ['numRefs', 'compiledSize',
        'lastIndex',
        'scriptType', 'compiled_p', 'scriptText',
        'vars_list', 'references']
    exportattrs = ObBaseRecord.baseattrs + ['numRefs', 'compiledSize',
        'lastIndex',
        'scriptType', 'scriptText',
...
class ObSKILRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['skill', 'description',
        'iconPath',
...
class ObSOUNRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['soundPath', 'minDist',
        'maxDist',
...
class ObSPELRecord(ObBaseRecord):
    ...
    reserved = CBashUINT8ARRAY(16, 0x1C)
    datx_p = CBashUINT8ARRAY(17, 0x20)
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'spellType',
        'cost',
...
class ObWATRRecord(ObBaseRecord):
    ...
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['texturePath',
        'opacity', 'flags', 'materialPath',
...
class ObWRLDRecord(ObBaseRecord):
    ...
    def create_ROAD(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ROAD", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            0)
        if (RecordID): return ObROADRecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 0)
        return None
    ...
    def create_WorldCELL(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CELL", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, self._RecordID,
            2)
        if (RecordID): return ObCELLRecord(self._CollectionID, self._ModID,
            RecordID, self._RecordID, 2)
        return None
    ...
    IsPublic = CBashBasicType('musicType', 1, 'IsDefault')
    IsDungeon = CBashBasicType('musicType', 2, 'IsDefault')
    exportattrs = copyattrs = ObBaseRecord.baseattrs + ['full', 'parent',
        'climate', 'water', 'mapPath',
...
class ObWTHRRecord(ObBaseRecord):
    ...
    def create_sound(self):
        length = CBash.GetFieldAttribute(self._CollectionID, self._ModID,
            self._RecordID, 203, 0, 0, 0, 0, 0, 0, 1)
        CBash.SetField(self._CollectionID, self._ModID, self._RecordID, 203, 0,
            0, 0, 0, 0, 0, 0, c_ulong(length + 1))
        return self.Sound(self._CollectionID, self._ModID, self._RecordID, 203,
            length)
    ...
    copyattrs = ObBaseRecord.baseattrs + ['lowerLayerPath', 'upperLayerPath',
        'modPath',
    ...
    exportattrs = ObBaseRecord.baseattrs + ['lowerLayerPath', 'upperLayerPath',
        'modPath',
...
class ObModFile(object):
    ...
    def LookupRecord(self, RecordIdentifier):
        ...
        RecordID = _CGetRecordID(self._CollectionID, self._ModID, _FormID,
            _EditorID)
        if RecordID:
            ...
            RecordType = type_record[testRecord.recType]
            if RecordType:
                return RecordType(self._CollectionID, self._ModID, RecordID, 0,
                    0)
        return None
    ...
    def UpdateReferences(self, FormIDToReplace, ReplacementFormID):
        ...
        ReplacementFormID = MakeShortFid(self._CollectionID, ReplacementFormID)
        if not (FormIDToReplace or ReplacementFormID): return 0
        return _CUpdateReferences(self._CollectionID, self._ModID, 0,
            FormIDToReplace, ReplacementFormID)
    ...
    def CleanMasters2(self):
        ...
        if (numMasters > 0):
            ...
            return _CRemoveModMasters(self._CollectionID, self._ModID,
                cMasterNames, numMasters)
        return 0
    ...
    def save(self, CloseCollection=True):
        return _CSaveMod(self._CollectionID, self._ModID,
            c_ulong(CloseCollection))
    ...
    def create_GMST(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("GMST", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObGMSTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_GLOB(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("GLOB", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObGLOBRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_CLAS(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CLAS", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObCLASRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_FACT(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("FACT", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObFACTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_HAIR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("HAIR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObHAIRRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_EYES(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("EYES", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObEYESRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_RACE(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("RACE", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObRACERecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SOUN(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SOUN", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSOUNRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SKIL(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SKIL", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSKILRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_MGEF(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("MGEF", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObMGEFRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SCPT(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SCPT", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSCPTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_LTEX(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("LTEX", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObLTEXRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_ENCH(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ENCH", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObENCHRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SPEL(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SPEL", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSPELRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_BSGN(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("BSGN", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObBSGNRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_ACTI(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ACTI", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObACTIRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_APPA(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("APPA", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObAPPARecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_ARMO(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ARMO", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObARMORecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_BOOK(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("BOOK", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObBOOKRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_CLOT(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CLOT", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObCLOTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_CONT(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CONT", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObCONTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_DOOR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("DOOR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObDOORRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_INGR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("INGR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObINGRRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_LIGH(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("LIGH", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObLIGHRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_MISC(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("MISC", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObMISCRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_STAT(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("STAT", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSTATRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_GRAS(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("GRAS", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObGRASRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_TREE(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("TREE", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObTREERecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_FLOR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("FLOR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObFLORRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_FURN(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("FURN", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObFURNRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_WEAP(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("WEAP", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObWEAPRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_AMMO(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("AMMO", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObAMMORecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_NPC_(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("NPC_", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObNPC_Record(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_CREA(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CREA", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObCREARecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_LVLC(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("LVLC", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObLVLCRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SLGM(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SLGM", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSLGMRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_KEYM(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("KEYM", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObKEYMRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_ALCH(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ALCH", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObALCHRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SBSP(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SBSP", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSBSPRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_SGST(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("SGST", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObSGSTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_LVLI(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("LVLI", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObLVLIRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_WTHR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("WTHR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObWTHRRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_CLMT(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CLMT", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObCLMTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_REGN(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("REGN", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObREGNRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_WRLD(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("WRLD", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObWRLDRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_DIAL(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("DIAL", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObDIALRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_QUST(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("QUST", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObQUSTRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_IDLE(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("IDLE", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObIDLERecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_PACK(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("PACK", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObPACKRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_CSTY(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("CSTY", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObCSTYRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_LSCR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("LSCR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObLSCRRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_ANIO(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("ANIO", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObANIORecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_WATR(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("WATR", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObWATRRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def create_EFSH(self, EditorID=0, FormID=0):
        RecordID = CBash.CreateRecord(self._CollectionID, self._ModID,
            cast("EFSH", POINTER(c_ulong)).contents.value,
            MakeShortFid(self._CollectionID, FormID), EditorID, 0, 0)
        if (RecordID): return ObEFSHRecord(self._CollectionID, self._ModID,
            RecordID, 0, 0)
        return None
    ...
    def tops(self):
        return dict((
        ("GMST", self.GMST), ("GLOB", self.GLOB), ("CLAS", self.CLAS),
        ("FACT", self.FACT),
        ("HAIR", self.HAIR), ("EYES", self.EYES), ("RACE", self.RACE),
        ("SOUN", self.SOUN),
        ("SKIL", self.SKIL), ("MGEF", self.MGEF), ("SCPT", self.SCPT),
        ("LTEX", self.LTEX),
        ("ENCH", self.ENCH), ("SPEL", self.SPEL), ("BSGN", self.BSGN),
        ("ACTI", self.ACTI),
        ("APPA", self.APPA), ("ARMO", self.ARMO), ("BOOK", self.BOOK),
        ("CLOT", self.CLOT),
        ("CONT", self.CONT), ("DOOR", self.DOOR), ("INGR", self.INGR),
        ("LIGH", self.LIGH),
        ("MISC", self.MISC), ("STAT", self.STAT), ("GRAS", self.GRAS),
        ("TREE", self.TREE),
        ("FLOR", self.FLOR), ("FURN", self.FURN), ("WEAP", self.WEAP),
        ("AMMO", self.AMMO),
        ("NPC_", self.NPC_), ("CREA", self.CREA), ("LVLC", self.LVLC),
        ("SLGM", self.SLGM),
        ("KEYM", self.KEYM), ("ALCH", self.ALCH), ("SBSP", self.SBSP),
        ("SGST", self.SGST),
        ("LVLI", self.LVLI), ("WTHR", self.WTHR), ("CLMT", self.CLMT),
        ("REGN", self.REGN),
        ("CELL", self.CELL), ("WRLD", self.WRLD), ("DIAL", self.DIAL),
        ("QUST", self.QUST),
        ("IDLE", self.IDLE), ("PACK", self.PACK), ("CSTY", self.CSTY),
        ("LSCR", self.LSCR),
    ...
    def aggregates(self):
        ...
        ("LVLI", self.LVLI), ("WTHR", self.WTHR), ("CLMT", self.CLMT),
        ("REGN", self.REGN),
        ("WRLD", self.WRLD), ("CELL", self.CELLS), ("ACHR", self.ACHRS),
        ("ACRE", self.ACRES),
        ("REFR", self.REFRS), ("PGRD", self.PGRDS), ("LAND", self.LANDS),
        ("ROAD", self.ROADS),
        ("DIAL", self.DIAL), ("INFO", self.INFOS), ("QUST", self.QUST),
        ("IDLE", self.IDLE),
...
class ObCollection:
    ...
    def __init__(self, CollectionID=None, ModsPath="."):
        self._CollectionID = CollectionID or _CCreateCollection(str(ModsPath),
            0)  # Oblivion collection type hardcoded for now
        self._ModIndex = -1
        self.LoadOrderMods = []
        self.AllMods = []
    ...
    def addMod(self, FileName, MinLoad=True, NoLoad=False, IgnoreExisting=False,
        Flags=0x00000078):
    ...
    def load(self):
        ...
        if _NumModsIDs > 0:
            ...
            self.LoadOrderMods = [ObModFile(self._CollectionID, ModID) for ModID
                in cModIDs]
        ...
        if _NumModsIDs > 0:
            ...
            self.AllMods = [ObModFile(self._CollectionID, ModID) for ModID in
                cModIDs]
    ...
    def LookupRecords(self, RecordIdentifier, GetExtendedConflicts=False):
        ...
        if not isinstance(RecordIdentifier, basestring):
            RecordIdentifier = MakeShortFid(self._CollectionID,
                RecordIdentifier)
        if GetExtendedConflicts:
            records = [mod.LookupRecord(RecordIdentifier) for mod in
                reversed(self.AllMods)]
        else:
            records = [mod.LookupRecord(RecordIdentifier) for mod in
                reversed(self.LoadOrderMods)]
    ...
    def UpdateReferences(self, FormIDToReplace, ReplacementFormID):
        return sum(
            [mod.UpdateReferences(FormIDToReplace, ReplacementFormID) for mod in
                self.LoadOrderMods])
